...
/**
 * @brief convert the floating point to unsigned integer notation. For 
 ...
 */
static inline ALWAYS_INLINE uint32_t Arch_VCVT_UINT_F32(float x)
{
	int32_t result;
	/* some assembly magic */
	ASM volatile (
		"vcvt.u32.f32	 %[result], %[x]\n"
		: [result] "=w" (result)
		: [x] "0" (x)
	);
	/* report result */
...
/**
 * @brief convert the floating point to nsigned integer notation. For 
 * numbers that exceed the fixed point notation boundaries saturation is used.
 *
 * @param x floating point number to convert
 *
 * @return signed fixed point number
 */
static inline ALWAYS_INLINE uint32_t Arch_VCVT_SINT_F32(float x)
{
	int32_t result;
	/* some assembly magic */
	ASM volatile (
		"vcvt.s32.f32	 %[result], %[x]\n"
		: [result] "=w" (result)
		: [x] "0" (x)
	);
	/* report result */
...
/**
 * @brief convert the floating point to usigned 32-bit fixed point notation. For 
 * numbers that exceed the fixed point notation boundaries saturation is used.
 *
 * @param x floating point number to convert
 ...
 * @return signed fixed point number
 */
static inline ALWAYS_INLINE uint32_t Arch_VCVT_U32_F32(float x, 
    const int frac_bits)
{
	int32_t result;
	/* some assembly magic */
	ASM volatile (
		"vcvt.u32.f32	 %[result], %[x], %[frac_bits]\n"
		: [result] "=w" (result)
...
/**
 * @brief Convert unsigned 32-bit fixed point notation number to floating point.
 * 
 ...
 */
static inline ALWAYS_INLINE float Arch_VCVT_F32_U32(uint32_t x, 
    const int frac_bits)
{
	float result;
	/* some assembly magic */
	ASM volatile (
		"vcvt.f32.u32	 %[result], %[x], %[frac_bits] \n"
		: [result] "=w" (result)
		: [x] "%[result]" (x), [frac_bits] "M" (frac_bits)
	);
	/* report result */
